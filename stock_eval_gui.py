...
def rev_trend():   # GUI用的函式袂使佇()內底寫參數。
    ...
    resp = requests.post(
        'https://mops.twse.com.tw/mops/web/t05st10_ifrs', form_data, headers=headers)
    resp.encoding = 'utf-8'
    ...
    try:
        ...
        if messagebox.askquestion(title='訊息', message='敢欲繼續評估別个項目？') == 'yes':
            cash_divs()
        else:
            # root.destroy()
            pass
...
def cash_divs():
    ...
    stk = entry.get()
    r = requests.get(
        'https://goodinfo.tw/tw/StockDividendPolicy.asp?STOCK_ID='+str(stk), headers=headers)
    r.encoding = 'utf-8'
    ...
    for y in tr_qual[0:5]:
        # 2023年第五个tr內底第十七个td；後一个tr內底仝款第十七个td
        cdr.append(y.find_all('td')[16].text.strip())
        pcr.append(y.find_all('td')[21].text.strip())
    labelVar.set(
        f'{stk}過去五冬的現金殖利率是{cdr[0]}%、{cdr[1]}%、{cdr[2]}%、{cdr[3]}%、{cdr[4]}%。\n（已經寫入去評估報告！）')
    labelVar.set(
        f'{stk}過去五冬的盈餘配息率是{pcr[0]}%、{pcr[1]}%、{pcr[2]}%、{pcr[3]}%、{pcr[4]}%。\n（已經寫入去評估報告！）')
    with open(file_path, 'a', encoding='utf-8') as f:
        f.write(
...
def getIS():
    ...
    stk = entry.get()
    r = requests.get(
        'https://goodinfo.tw/tw/StockFinDetail.asp?RPT_CAT=IS_M_QUAR&STOCK_ID='+str(stk), headers=headers)
    ...
    for tr in iss.find_all('tr'):
        if tr.find_all('td') != []:
            if '營業毛利' == tr.find('nobr').text.strip():
                ...
                gmrs = [gmr[2].text.strip(), gmr[4].text.strip(), gmr[6].text.strip(
                ), gmr[8].text.strip(), gmr[10].text.strip(), gmr[12].text.strip(), gmr[14].text.strip()]
            ...
            if '營業利益' == tr.find('nobr').text.strip():
                opr = tr.find_all('td')
                oprs = [opr[2].text.strip(), opr[4].text.strip(), opr[6].text.strip(
                ), opr[8].text.strip(), opr[10].text.strip(), opr[12].text.strip(), opr[14].text.strip()]
    ...
    # 粗估今年EPS
            if '每股稅後盈餘(元)' == tr.find('nobr').text.strip():
                epss = tr.find_all('td')
                eps = [epss[1].text.strip(), epss[3].text.strip(), epss[5].text.strip(
                ), epss[7].text.strip(), epss[9].text.strip(), epss[11].text.strip(), epss[13].text.strip()]
...
def getCF():
    ...
    messagebox.showinfo(title="訊息", message="評估結束，請參考評估報告！")
